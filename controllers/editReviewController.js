...
const updateImages = async (base64Images,images) => {
  ...
  const deletedReviewImages=[...images];
  ...
  const uploadedImageUrls = await Promise.all(
    base64Images.map(async (base64String) => {
      ...
      if (base64String.startsWith("data:image")) {
        ...
        deletedReviewImages=deletedReviewImages.filter(image=>image.imageUrl!=base64String);

        unchangedReviewImages=deletedReviewImages.filter(image=>image.imageUrl==base64String);
      }
    })
  );
  
  return {
    reviewImages:[...unchangedReviewImages,...uploadedImageUrls],
    deletedReviewImages
  }
...
const editReview = async (req, res) => {
  try {
    try {
      ...
      const {deletedReviewImages,reviewImages} = await updateImages(images,previousImages);
      ...
      res
        .status(201)
        .json({ message: "Review is uccessfully created!", review });
    } catch (error) {
